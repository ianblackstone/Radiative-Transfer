...
photon_state[:,4] = 2*np.pi*np.random.rand(len(photon_state))
...
def CheckBoundary(photon_state,boundary):
    ...
    if len(boundary_photons)>0:
        ...
        if boundary == 'reemit':
            ...
            photon_state[boundary_photons,4] = 2*np.pi*np.random.rand(len(boundary_photons))
...
# binned_mu = np.floor(np.multiply(escaped_mu,num_bins)) + 1
# bins, count = np.unique(binned_mu, return_counts=True)

# midpoints = np.pi/(2*num_bins)*np.ones(num_bins)

# midpoints = np.cumsum(midpoints) + np.pi/(2*num_bins)

count = count*num_bins/(2*num_photons*np.cos(midpoints))

plt.figure(2)
plt.scatter(midpoints,count)
# plt.hist(theta_f,bins=10,density=True)
plt.xlabel(r'$\theta$')
